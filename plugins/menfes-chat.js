...
`
    ...
    switch (command) {
        ...
        case 'menfesleave': {
            ...
            if (!room) return this.sendButton(m.chat, '*Kamu tidak sedang berada di menfes chat*', author, null, [['Cari Balasan', `.menfesstart`]], m)
            m.reply('Ok')
            let other = room.other(m.sender)
            if (other) await this.sendButton(other, '*Balasan meninggalkan chat*', author, null, [['Cari Balasan', `.menfesstart`]], m)
	...
	await conn.sendButton(m.chat, suks, wm, null, [['Menu', '/menu']], m, { mentions: conn.parseMention(suks) })
            // Batas
                await this.sendButton(room.a, '*Balasan ditemukan!*', author, null, [['Next', `.menfesnext`]], m)
                room.b = mention
                room.state = 'CHATTING'
                await this.sendButton(room.a, '*Balasan ditemukan!*', author, null, [['Next', `.menfesnext`]], m)
            } else {
